...
class TableQA:
    ...
    def _build_model(self):
        if self.opt["mode"]=='train':
            ...
            self.model = Graph2Seq.load_checkpoint(self.opt["checkpoint_save_path"],
                                                   "best_bert_wikisql.pt" if self.opt["dataset"]=="WikiSQL"\
                                                   else "best_bert_wtq.pt").to(
                self.device
            )
            ...
            if epoch >= 0:
                ...
                if score >= max_score:
                    ...
                    self.model.save_checkpoint(self.opt["checkpoint_save_path"], 
                                               "best_bert_wikisql.pt" if self.opt["dataset"]=="WikiSQL"\
                                                   else "best_bert_wtq.pt")
                    self._best_epoch = epoch
                max_score = max(max_score, score)
            if epoch >= 10 and self._stop_condition(epoch):
                break
        return max_score
    ...
    def evaluate(self, split="test"):
        ...
        if split in ["train_sql","test","dev1","dev2","dev3",
                         "perturb_test_orig","perturb_test_changed",
                         "perturb_dev_orig","perturb_dev_changed"]:
            save_dir = "wikisql" if self.opt["dataset"]=='WikiSQL' else 'wtq'
            with open(f'output/bert_{save_dir}_{split}_gt.txt', 'w') as fp:
                for item in gt_collect:
                    # write each item on a new line
                    fp.write("%s\n" % item)
                print('Done')
            with open(f'output/bert_{save_dir}_{split}_pred.txt', 'w') as fp:
