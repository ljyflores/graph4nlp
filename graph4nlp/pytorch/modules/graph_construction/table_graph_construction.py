...
class TableGraphConstruction(StaticGraphConstructionBase):
    """
    ...
    def __init__(
        ...
        super(TableGraphConstruction, self).__init__()
        self.vocab = vocab
        self.verbose = 1
    ...
    @classmethod
    def parsing(cls, raw_question, raw_table):
        """
        ...
        NUM_COLS = len(raw_table.columns[1:])
        NUM_ROWS = raw_table.shape[0]
        pd_table = raw_table.fillna("").applymap(str)
        entity_lst = list(chain.from_iterable(pd_table.values.T))

        # Initialize nodes
        node_content = []
        node_content.append({'token': raw_question, 'position_id': 0, 'id': 0, 'sentence_id': 0})
        for idx, ent in enumerate(entity_lst):
            node_content.append({'token': ent, 'position_id': idx+1, 'id': idx+1, 'sentence_id': idx+1})
        
        num_nodes = len(node_content)

        # Initialize edges
        src_lst, tgt_lst, rel_lst = [], [], []
        emb_cols = [str(x) for x in pd_table.columns]

        # Add edges from question to entity
        src_lst.extend([0]*NUM_ROWS)
        tgt_lst.extend(list(range(1,NUM_ROWS+1)))
        rel_lst.extend([emb_cols[0]]*NUM_ROWS)
        
        # Add edges from entity to attribute
        src_lst.extend(torch.tensor(range(1,NUM_ROWS+1)).repeat_interleave(NUM_COLS).numpy())
        for i in range(1,NUM_ROWS+1):
            temp = list((np.array(range(1,NUM_COLS+1))*NUM_ROWS)+i)
            tgt_lst.extend(temp)
            rel_lst.extend(emb_cols[1:NUM_COLS+1])

        # Format into dictionaries
        graph_content = [{'edge_type': r, 
                          'src': int(s), 
                          'tgt': int(t)} for (r,s,t) in zip(rel_lst, src_lst, tgt_lst)]
        parsed_results = {'graph_content': graph_content,
    ...
    def static_topology(
        ...
        for parsed_sent in parsed_results:
            ...
            parsed_sent, edge_strategy='heterogeneous', sequential_link=sequential_link
            )
            sub_graphs.append(graph)

        joint_graph = cls._graph_connect(sub_graphs, merge_strategy=None)
        return joint_graph
    ...
    def _construct_static_graph(cls, parsed_object, edge_strategy=None, sequential_link=True):
        ...
        head_node = 0
        tail_node = len(parsed_object["node_content"]) - 2
